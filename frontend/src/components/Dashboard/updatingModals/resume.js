...
import { uploadResumeFileMongoDB } from "../../../APIs/user";
...
const ModalComponent = ({ email, onClose }) => {
  ...
  const uploadResumeFileFirebase = () => {
    if (!file) {
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref)
          .then(async (downloadURL) => {
            setUrl(downloadURL);
            try {
              const res = await uploadResumeFileMongoDB({
                Email: email,
                url: downloadURL,
              });
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-6 rounded-lg shadow-lg w-96">
        ...
        <p className="text-red-500 text-sm ml-4 ">
          * File type should be PDF and size should be less than 5 MB.
        </p>
        <div className="flex justify-end mt-4">
          {file && file.size <= 5 * 1024 * 1024 && file.type.includes("pdf") ? (
            <button
              onClick={uploadResumeFileFirebase}
